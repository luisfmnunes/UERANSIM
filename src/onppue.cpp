...
int main(int argc, char **argv)
{
    ...
    auto onppue = std::make_unique<onpp::ue::OMNetUEListerner>();

    for (int i = 0; i < g_options.count; i++)
    {
        auto *config = GetConfigByUe(i);
        auto *ue = new nr::ue::UserEquipment(config, &g_ueController, onppue.get(), g_cliRespTask);
